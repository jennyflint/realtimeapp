                ...
                <div class="p-0 sm:p-6">
                    ...
                    <div class="divide-y divide-gray-200">
                        @foreach ($users as $user)
                            <div class="grid grid-cols-1 md:grid-cols-12 p-4 md:px-6 md:py-4 items-center hover:bg-gray-50 transition duration-150 {{ $user->unread_count > 0 ? 'bg-indigo-50/30' : '' }}">

                                <div class="col-span-1 md:col-span-4 flex items-center mb-2 md:mb-0">
                                    <div class="relative h-10 w-10 shrink-0">
                                        <div class="h-full w-full rounded-full {{ $user->unread_count > 0 ? 'bg-indigo-600' : 'bg-indigo-500' }} flex items-center justify-center text-white font-bold shadow-sm">
                                            {{ strtoupper(substr($user->name, 0, 1)) }}
                                        </div>
                                        
                                        @if($user->unread_count > 0)
                                            <span class="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-600 text-[10px] font-bold text-white border-2 border-white animate-pulse">
                                                {{ $user->unread_count }}
                                            </span>
                                        @endif
                                    </div>
                                    ...
                                    <div class="ml-4">
                                        <div class="text-sm {{ $user->unread_count > 0 ? 'font-bold text-indigo-900' : 'font-medium text-gray-900' }}">
                                            {{ $user->name }}
                                        </div>
